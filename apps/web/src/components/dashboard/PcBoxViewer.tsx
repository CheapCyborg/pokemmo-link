...
function PcBoxViewerComponent({ onOpenDetails }: PcBoxViewerProps) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Content Area */}
      {state.flowState === "loading" || state.flowState === "enriching" ? (
        ...
        <div className="grow h-full">
          <VirtuosoGrid
            key={state.activeBoxId} // Reset internal state when box changes to avoid
            useWindowScroll
            ...
            itemContent={(index, p) => (
              <PokemonCard.Root key={`${p.identity.uuid}-${index}`} pokemon={p} onClick={() => onOpenDetails(p)}>
                <PokemonCard.Header context="pc" />
                <PokemonCard.Body />
                <PokemonCard.Footer />
              </PokemonCard.Root>
            )}
