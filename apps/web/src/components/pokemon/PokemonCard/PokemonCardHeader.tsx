...
export function PokemonCardHeader({ showOwner = false, context, className }: PokemonCardHeaderProps) {
  ...
  return (
    <div
      className={cn(
        "p-2 flex flex-col flex-auto gap-1 bg-linear-to-br from-gray-50 to-gray-100 dark:from-slate-800 dark:to-slate-900 border-b border-gray-100 dark:border-slate-700 relative group-hover:from-indigo-100 group-hover:to-indigo-50 dark:group-hover:from-indigo-950/40 dark:group-hover:to-slate-800 transition-colors duration-300",
        className
      )}>
      {/* Top Row: Dex/Status + Level/IV - Fixed Height (h-7 = 28px) to prevent layout shift */}
      <div className="flex justify-between items-start w-full shrink-0 h-7">
      ...
      <div className="flex items-center gap-2 grow min-h-0">
        ...
        <div className="min-w-0 flex flex-col gap-0.5">
          ...
          {/* Species Name */}
          {showSpeciesName && (
            <p className="text-[9px] text-gray-500 dark:text-gray-400 font-medium uppercase tracking-wide truncate leading-tight -mt-1">
              {speciesName}
            </p>
          )}

          {/* Badges Row */}
          <div className="flex flex-wrap gap-1 content-start overflow-hidden shrink-0 mt-0.5">
          ...
          {/* Ability */}
          <div className="flex items-baseline gap-1 text-[11px] leading-tight overflow-hidden shrink-0 mt-0.5">
