...
/**
 * Runtime configuration manager that loads settings from JSON. Eliminates hardcoded constants and
...
object ConfigManager {
    ...
    data class DebugConfig(
            ...
            val verboseDiscovery: Boolean =
                    true, // Log EVERY packet instance (not just unique classes)
    ...
    data class OutputConfig(
            val path: String =
                    System.getProperty("project.dir")?.let { "$it/../web/data" } ?: "../web/data",
            val useAbsolutePath: Boolean = System.getProperty("project.dir") != null,
            val absolutePath: String =
                    System.getProperty("project.dir")?.let { "$it/../web/data" } ?: ""
    )

    private val gson: Gson = GsonBuilder().setPrettyPrinting().create()

    // Always use project directory from Gradle (not current working directory which changes when
    // PokeMMO runs)
    ...
    fun load(file: File = configFile): Config {
        ...
        return if (file.exists()) {
            ...
            Logger.warn(
                    "Config file not found, creating default at ${file.absolutePath}",
                    "ConfigManager"
            )
            createDefaultConfig(file)
        }
    ...
    fun getLogLevel(): Logger.Level {
        return when (config.debug.logLevel.uppercase()) {
            ...
            "ERROR" -> Logger.Level.ERROR
                ...
                Logger.warn(
                        "Invalid log level '${config.debug.logLevel}', defaulting to INFO",
                        "ConfigManager"
                )
                Logger.Level.INFO
            }
    ...
    fun getOutputDir(): File {
        val outputPath =
                if (config.output.useAbsolutePath && config.output.absolutePath.isNotEmpty()) {
